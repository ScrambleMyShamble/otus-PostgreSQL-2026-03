...
Такое поведение объясняется тем, что на уровне READ COMMITTED снимки данных берутся на момент выполненя команд, например select, каждая новая команда, если
в данных были изменения, выдаст новый результат.
...
В этом и есть главное отличие таких уровней как REPEATABLE READ и READ COMMITED. Они делают снимки на разные моменты времени, в итоге мы получаем разное поведение. 
