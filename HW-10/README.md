...
## 6. Итог и краткие комментарии по каждому индексу
* idx_products_price - Обычный B-tree индекс для быстрого поиска по ценовому диапазону
* idx_products_search - GIN индекс для эффективного полнотекстового поиска
* idx_active_high_price - Частичный индекс для уменьшения размера и ускорения запросов только к активным товарам
* idx_lower_name - Функциональный индекс для регистронезависимого поиска
* idx_category_price - Составной индекс для запросов с фильтрацией по двум полям
